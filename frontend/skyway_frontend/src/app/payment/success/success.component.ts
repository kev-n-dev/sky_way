...
export class SuccessComponent   implements OnInit {
  ...
  constructor(
    ...
    console.log('SuccessComponent Constructor Called');
  }
  ...
  ngOnInit(): void {
    ...
    this.bookingService.viewBookings(null, null, this.bookingId).subscribe(
      (response: Booking[]) => {
        console.log('got booking details:', response);
        this.bookingReferenceId = response[0].reference_number
        this.flightDetails = {
          from: response[0].departure_flight.departure_airport.name,
          to: response[0].departure_flight.arrival_airport.name,
          date: response[0].departure_flight.start_date,
          time: response[0].departure_flight.departure_time,
        };
        this.passengerList = response[0].passengers.map(passenger => passenger.first_name + " "+ passenger.last_name).join(', ');
  ...
  downloadPDF() {
    ...
    const content = `
<div style="font-family: Arial, sans-serif; margin: 20px; padding: 20px; border: 2px dashed #dc2626; border-radius: 10px; background-color: #f9f9f9; text-align: left; width: 90vw!important">
  ...
  <div style="margin-top: 20px; padding: 10px; border: 1px solid #ccc; border-radius: 5px; background-color: #fff; display: flex; align-items: center; width: 85vw">
    ...
    <div style="text-align: left;">
      ...
      <p><strong>Passenger List:</strong> ${this.passengerList}</p>
    </div>
